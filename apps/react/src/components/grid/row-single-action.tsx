    ...
    <div className="demo-content">
      ...
      <div className="content">
        <CdsGrid className="demo-grid">
          ...
          {data.map((item: any) => (
            <CdsGridRow key={item.id}>
              <CdsGridCell>
                <CdsButtonAction
                  aria-label="choose available host actions"
                  onClick={event => rowActionOnClick(event, item)}
                ></CdsButtonAction>
              </CdsGridCell>
        ...
        <CdsDropdown hidden={rowActionDrowdownHidden ? true : undefined} anchor={rowActionAnchor}>
          <CdsButton block action="flat" size="sm">
            Shutdown {rowActionItem?.id}
          </CdsButton>
          <CdsButton block action="flat" size="sm">
            Restart {rowActionItem?.id}
          </CdsButton>
        </CdsDropdown>
      </div>
    </div>
