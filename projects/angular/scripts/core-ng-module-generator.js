...
const SOURCE_PATH = path.join(__dirname, './../../core/dist/core/custom-elements.json');
const TARGET_PATH = path.join(__dirname, './../../angular/projects/cds-angular');
const ROOT_SOURCE_PATH = path.join(__dirname, './../../angular/projects/cds-angular/src');
const MUSTACHE_TEMPLTATE_PATH = path.join(__dirname, './../../angular/projects/cds-angular/_stubs');
...
const licenseHeader = `${fs.readFileSync('./../../.license-header.js', 'utf8')}\n`;
...
function writeRootPublicAPI(modules) {
  writeToDisk(
    path.join(ROOT_SOURCE_PATH, `public-api.ts`),
    `${licenseHeader}${Mustache.render(rootIndexTemplate, { modules })}`
  );
}

function writeRootModule(modules) {
  writeToDisk(
    path.join(ROOT_SOURCE_PATH, MODULE_NAME),
    `${licenseHeader}${Mustache.render(rootModuleTemplate, { modules })}`
  );
}
