...
export function sizeDimensionKeyframes(animationKeyframes: AnimationKeyframes, hostEl: Element): AnimationKeyframes {
  ...
  return animationKeyframes.map((kf: Keyframe) => {
    if (
      kf &&
      Object.prototype.hasOwnProperty.call(kf, 'height') &&
      isPrefixedBy(kf?.height?.toString() || '', 'from:')
    ) {
    ...
    if (kf && Object.prototype.hasOwnProperty.call(kf, 'width') && isPrefixedBy(kf?.width?.toString() || '', 'from:')) {
